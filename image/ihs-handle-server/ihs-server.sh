...
mysql --user=${MARIADB_USER} --password=${MARIADB_PASSWORD} --host=${MARIADB_HOST} --batch ${MARIADB_DATABASE} < create_tables.sql
sed -i "s/PASSWORD/${MARIADB_PASSWORD}/g" insert_into.sql
sed -i "s/PREFIX/${INDEPENDENT_HANDLE_SERVER_PREFIX}/g" insert_into.sql
mysql --user=${MARIADB_USER} --password=${MARIADB_PASSWORD} --host=${MARIADB_HOST} --batch ${MARIADB_DATABASE} < insert_into.sql
./bin/hdl-server /ihs
