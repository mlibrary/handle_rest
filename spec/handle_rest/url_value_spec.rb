...
describe HandleRest::UrlValue do
  ...
  context "when url invalid form" do
    ...
    it "#self.to_s raises exception" do
      expect { url_value.to_s }.to raise_exception(RuntimeError, "URL value '#{url}' invalid form.")
    end

    it "#self.from_s raises exception" do
      expect { described_class.from_s(url) }.to raise_exception(RuntimeError, "URL value '#{url}' invalid form.")
    end

    it "#self.from_h raises exception" do
      expect { described_class.from_h("string", url) }.to raise_exception(RuntimeError, "URL value '#{url}' invalid form.")
    end
  end
end
