module HandleRest
  class Service
    def initialize(handle_service_rest_url,
      ...
      raise "non-identifier: naming authority identifier" unless naming_authority_identifier.is_a?(Handle)
      raise "non-identity: admin identity" unless admin_identity.is_a?(Identity)
      raise "non-identifier: root naming authority identifier" unless root_naming_authority_identifier.is_a?(Handle)
    ...
    def get_url(handle_identifier)
      raise "non-identifier: handle identifier" unless handle_identifier.is_a?(Handle)
